...
# (제자리 정렬이므로 새 리스트를 반환하지 않음)

# int 는 왜 쓰는가?
# - int = 정수 타입(…, -2, -1, 0, 1, 2, …).
# - ts_index: int = 0  → ts_index 라는 매개변수는 “정수여야 한다”는 ‘타입힌트’.
# - “인덱스(칸 번호)”는 0, 1, 2 처럼 정수만 의미가 있으니까 int 로 박아둔다.
# - 타입힌트는 실행 중 강제는 아님(런타임 에러로 막아주진 않음). 대신:
#   * IDE 자동완성/경고 강화
#   * 정적체커(mypy/pyright)로 사전 오류 탐지
#   * 팀원/미래의 나에게 코드 의도 전달
                                    ...
                                    # 리스트를 자체를 제자리에서 정렬해(.sort). 최신 시간이 앞으로 오게 reverse=True
                                    # key=... : 정렬 기준값을 뽑는 함수
                                    # - lambda x: ... 여기서 x는 한 줄(튜플)이고,
                                    # - x[ts_index]   타임스탬프 문자열ㅇ르 꺼낸 다음,
                                    # - datetime.strptime(문자열, DT_FMT) 로 "진짜 시간객체"로 바꿔서 정확히 비교.